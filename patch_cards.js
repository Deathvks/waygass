...
const regex = /<div className="glass-core border border-slate-200\/50 dark:border-white\/5 rounded-2xl overflow-hidden flex flex-col">[\s\S]*?\{usersList\.map\(\(u, i\) => \([\s\S]*?<div key=\{u\.id\} className="p-4 border-b border-slate-200\/50 dark:border-white\/5 last:border-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-1">[\s\S]*?<div className="min-w-0 flex-1 flex flex-col overflow-hidden">[\s\S]*?<div className="font-bold text-sm text-slate-800 dark:text-white flex items-center gap-2">[\s\S]*?<span className="truncate flex-shrink">\{u\.name\} \{u\.lastName\}<\/span>[\s\S]*?<div className="flex shrink-0 gap-1">[\s\S]*?\{u\.role === 'admin' && <span className="bg-primary text-white text-\[8px\] px-1\.5 py-0\.5 rounded uppercase font-black">Admin<\/span>\}[\s\S]*?\{u\.subscription === 'pro' && <span className="bg-primary\/20 text-primary text-\[8px\] px-1\.5 py-0\.5 rounded uppercase font-black">Pro<\/span>\}[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<div className="text-\[11px\] text-slate-500 truncate w-full">\{u\.email\}<\/div>[\s\S]*?<\/div>[\s\S]*?<div className="flex items-center gap-1 shrink-0 self-end sm:self-auto">[\s\S]*?<button onClick=\{\(\) => setRoleModalUser\(u\)\} className="w-8 h-8 rounded-full bg-slate-100\/50 dark:bg-white\/5 flex items-center justify-center text-slate-500 hover:text-primary transition">[\s\S]*?<svg className="w-3\.5 h-3\.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8V7z"\/><\/svg>[\s\S]*?<\/button>[\s\S]*?<button onClick=\{\(\) => setDeleteModalUser\(u\)\} className="w-8 h-8 rounded-full bg-slate-100\/50 dark:bg-white\/5 flex items-center justify-center text-slate-500 hover:text-red-500 transition">[\s\S]*?<svg className="w-3\.5 h-3\.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-\.867 12\.142A2 2 0 0116\.138 21H7\.862a2 2 0 01-1\.995-1\.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"\/><\/svg>[\s\S]*?<\/button>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?\)\)}[\s\S]*?<\/div>/m;
                          ...
                          <div key={u.id} className="relative group bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 rounded-2xl p-4 flex flex-col gap-4 hover:border-primary/40 transition-all hover:shadow-lg hover:shadow-primary/5">
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="font-bold text-[13px] text-slate-800 dark:text-white truncate">{u.name} {u.lastName}</div>
                                <div className="text-[11px] text-slate-500 truncate mt-0.5" title={u.email}>{u.email}</div>
                                <div className="text-[9px] text-slate-400 mt-1 font-medium">Desde {new Date(u.createdAt).toLocaleDateString('es-ES', { month: 'short', year: 'numeric' })}</div>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-200/50 dark:border-white/5">
                              <div className="flex flex-wrap gap-1.5">
                                {u.authProvider === 'google' ? (
                                  <span className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 text-[9px] px-2 py-0.5 rounded-full font-bold flex items-center gap-1 shadow-sm" title="Google"><svg className="w-2.5 h-2.5 text-blue-500" viewBox="0 0 24 24"><path fill="currentColor" d="M12.545,10.239v3.821h5.445c-0.712,2.315-2.647,3.972-5.445,3.972c-3.332,0-6.033-2.701-6.033-6.032s2.701-6.032,6.033-6.032c1.498,0,2.866,0.549,3.921,1.453l2.814-2.814C17.503,2.988,15.139,2,12.545,2C7.021,2,2.543,6.477,2.543,12s4.478,10,10.002,10c8.396,0,10.249-7.85,9.426-11.748L12.545,10.239z"/></svg> Google</span>
                                ) : (
                                  <span className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 text-[9px] px-2 py-0.5 rounded-full font-bold flex items-center gap-1 shadow-sm" title="Email/Contraseña"><svg className="w-2.5 h-2.5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg> Email</span>
                                )}
