...
export default function SettingsModal({ 
  ...
  const [syncProgress, setSyncProgress] = React.useState({ percent: 0, message: '' });
  ...
  const handleTriggerCron = async () => {
    ...
    try {
      ...
      toast.success("Gasolineras descargadas con éxito");
  ...
  const themeModal = pendingTheme && (
    <div className="fixed inset-0 z-[70] flex itemás-center justify-center p-4 bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm">
  ...
  return (
    <>
      {themeModal}
      <div className="fixed inset-0 z-[60] flex itemás-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white/95 dark:bg-[#0c0c0e]/95 backdrop-blur-3xl rounded-[32px] border border-slate-200/50 dark:border-white/10 shadow-none overflow-y-auto max-h-[90vh] animate-in fade-in zoom-in-95 duration-200 no-scrollbar">
        ...
        <div className="sticky top-0 z-10 bg-white/95 dark:bg-[#0c0c0e]/95 backdrop-blur-3xl px-6 pt-[max(env(safe-area-inset-top),1.5rem)] pb-4 border-b border-slate-200/50 dark:border-white/5">
          <div className="flex itemás-center justify-between mb-4">
            <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight flex itemás-center gap-2">
              {activeTab === 'settings' ? 'Ajustes' : activeTab === 'ranking' ? 'Comunidad' : 'Administración'}
            </h2>
            <button onClick={onClose} className="w-8 h-8 flex itemás-center justify-center bg-slate-100/50 dark:bg-white/5 text-slate-500 hover:text-slate-900 dark:hover:text-white rounded-full transition">
          ...
          <div className="flex bg-slate-100/50 dark:bg-white/5 p-1 rounded-xl">
            ...
            {user?.role === 'admin' && (
              <button onClick={() => setActiveTab('admin')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-colors flex itemás-center justify-center gap-1 ${activeTab === 'admin' ? 'bg-primary text-white shadow-none' : 'text-slate-500 hover:text-slate-700 dark:hover:text-white'}`}>Admin</button>
            )}
          </div>
        </div>
        ...
        <div className="p-6 pt-2">
          {activeTab === 'settings' && (
            <div className="glass-core border border-slate-200/50 dark:border-white/5 rounded-2xl overflow-hidden flex flex-col">
              ...
              <div className="flex flex-col p-4 border-b border-slate-200/50 dark:border-white/5 gap-3">
                <label className="text-sm font-bold text-slate-800 dark:text-white flex itemás-center gap-2">
                  <svg className="w-4 h-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                  Gasto Habitual (€)
                </label>
                <p className="text-xs text-slate-500 dark:text-slate-400">Calcularemos cuántos litros te caben y cuánto más echas en comparación con la media de la zona.</p>
                <div className="flex itemás-center gap-3">
                  <input 
                    type="number" 
                    min="1" 
                    max="500" 
              ...
              <div className="flex flex-col p-4 border-b border-slate-200/50 dark:border-white/5 gap-3">
                ...
                <div className={`flex flex-col gap-2 mt-1 ${(!user || user.subscription === 'free') ? 'opacity-50 pointer-events-none' : ''}`}>
                  <div className="flex itemás-center justify-between">
                    <div className="flex itemás-center gap-2">
                      <div className={`w-5 h-5 rounded-full flex itemás-center justify-center text-[9px] font-bold ${settings.cardWaylet ? 'bg-[#ff8200] text-white' : 'bg-slate-200 dark:bg-white/10 text-slate-500'}`}>W</div>
                      <span className="text-sm font-semibold text-slate-600 dark:text-slate-300">Waylet</span>
                    </div>
                    <button onClick={() => setSettings({ ...settings, cardWaylet: !settings.cardWaylet })} className={`w-9 h-5 flex itemás-center rounded-full p-1 duration-300 ease-in-out ${settings.cardWaylet ? 'bg-primary' : 'bg-slate-200 dark:bg-slate-700'}`}>
                  ...
                  <div className="flex itemás-center justify-between">
                    <div className="flex itemás-center gap-2">
                      <div className={`w-5 h-5 rounded-full flex itemás-center justify-center text-[9px] font-bold ${settings.cardCepsa ? 'bg-[#e4002b] text-white' : 'bg-slate-200 dark:bg-white/10 text-slate-500'}`}>C</div>
                      <span className="text-sm font-semibold text-slate-600 dark:text-slate-300">Cepsa Gow</span>
                    </div>
                    <button onClick={() => setSettings({ ...settings, cardCepsa: !settings.cardCepsa })} className={`w-9 h-5 flex itemás-center rounded-full p-1 duration-300 ease-in-out ${settings.cardCepsa ? 'bg-primary' : 'bg-slate-200 dark:bg-slate-700'}`}>
              ...
              {/* Legal Row */}
              <div className="p-4 flex flex-col itemás-center justify-center gap-1">
          ...
          {activeTab === 'ranking' && (
            <div className="flex flex-col gap-4">
              <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4 flex gap-3 itemás-start">
              ...
              {loadingRanking ? (
                ...
                <div className="glass-core border border-slate-200/50 dark:border-white/5 rounded-2xl overflow-hidden flex flex-col">
                  {rankingData.map((u, index) => (
                    <div key={u.id} className="flex itemás-center p-4 border-b border-slate-200/50 dark:border-white/5 last:border-0 gap-3">
                      <div className={`w-8 h-8 rounded-full flex itemás-center justify-center font-black ${index === 0 ? 'bg-primary text-white' : index === 1 ? 'bg-slate-300 text-slate-700' : index === 2 ? 'bg-primary/20 text-primary' : 'bg-slate-100/50 dark:bg-white/5 text-slate-500'}`}>
                        {index + 1}
                      </div>
                ...
                <div className="glass-core border border-slate-200/50 dark:border-white/5 rounded-2xl p-8 flex flex-col itemás-center justify-center text-center">
                  <div className="w-12 h-12 bg-slate-100 dark:bg-white/5 rounded-full flex itemás-center justify-center mb-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="glass-core border border-slate-200/50 dark:border-white/5 p-4 rounded-2xl col-span-2 flex flex-col gap-3">
                          <div className="flex justify-between itemás-center">
                            ...
                            <button onClick={handleTriggerCron} disabled={isSyncing} className="bg-primary text-white text-[10px] font-bold px-3 py-1.5 rounded-lg hover:bg-primary-dark transition-colors shrink-0 flex itemás-center justify-center min-w-[110px] disabled:opacity-50">
                              {isSyncing ? <div className="flex gap-1.5 itemás-center"><div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div><span>{syncProgress.percent}%</span></div> : 'Descargar Ahora'}
                            </button>
                          </div>
                          
                          {isSyncing && (
                            <div className="w-full mt-1">
                              <div className="w-full bg-slate-200 dark:bg-white/10 rounded-full h-1.5 mb-1 overflow-hidden">
                                <div className="bg-primary h-1.5 rounded-full transition-all duration-300" style={{ width: `${syncProgress.percent}%` }}></div>
                              </div>
                              <div className="text-[9px] text-slate-500 dark:text-slate-400 font-medium mt-1 text-right">{syncProgress.message}</div>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="glass-core border border-slate-200/50 dark:border-white/5 rounded-2xl overflow-hidden flex flex-col">
                      {usersList.map((u, i) => (
                        <div key={u.id} className="p-4 border-b border-slate-200/50 dark:border-white/5 last:border-0 flex flex-col sm:flex-row sm:itemás-center justify-between gap-3 sm:gap-1">
                            <div className="min-w-0 flex-1 flex flex-col overflow-hidden">
                              <div className="font-bold text-sm text-slate-800 dark:text-white flex itemás-center gap-2">
                            ...
                            <div className="flex itemás-center gap-1 shrink-0 self-end sm:self-auto">
                            <button onClick={() => setRoleModalUser(u)} className="w-8 h-8 rounded-full bg-slate-100/50 dark:bg-white/5 flex itemás-center justify-center text-slate-500 hover:text-primary transition">
                            ...
                            <button onClick={() => setDeleteModalUser(u)} className="w-8 h-8 rounded-full bg-slate-100/50 dark:bg-white/5 flex itemás-center justify-center text-slate-500 hover:text-red-500 transition">
      ...
      {roleModalUser && (
        <div className="fixed inset-0 z-[70] flex itemás-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm glass-core border border-slate-200/50 dark:border-white/10 rounded-3xl p-6 shadow-none animate-in zoom-in-95 duration-200">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => handleChangeRole('admin')} className="p-4 rounded-xl flex itemás-center justify-between border border-slate-200/50 dark:border-white/10 bg-slate-100/50 dark:bg-white/5 hover:bg-primary/10 hover:border-primary/50 transition">
              ...
              <button onClick={() => handleChangeRole('user')} className="p-4 rounded-xl flex itemás-center justify-between border border-slate-200/50 dark:border-white/10 bg-slate-100/50 dark:bg-white/5 hover:bg-primary/10 hover:border-primary/50 transition">
