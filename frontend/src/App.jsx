...
export default function App() {
  ...
  const detectProvince = async (lat, lng) => {
    ...
    try {
      ...
      const res = await axios.get(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&email=hola@waygas.com`);
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      if (res.data && res.data.length > 0) {
        const loc = { lat: parseFloat(res.data[0].lat), lng: parseFloat(res.data[0].lon) };
        setUserLocation(loc);
        await detectProvince(loc.lat, loc.lng);
      }
...
dark:text-slate-100 relative">
      ...
      {/* Panel Izquierdo (Móvil: scroll completo / Desktop: Panel Flotante) */}
      <div className="flex flex-col w-full lg:w-[420px] xl:w-[460px] relative lg:absolute lg:top-4 lg:bottom-4 lg:left-4 z-10 bg-transparent lg:bg-[#FFF9F5] dark:lg:bg-slate-900 lg:shadow-[0_20px_60px_rgba(0,0,0,0.15)] overflow-visible lg:overflow-hidden lg:rounded-[32px] pb-24 lg:pb-0 lg:sheet-scroll">
        
        <div className="relative z-10 flex flex-col min-h-full lg:min-h-0 lg:h-full">
          <div className="brand-gradient-bg rounded-b-[40px] flex-shrink-0 flex flex-col pb-6">
            <Header 
            ...
            <div className="w-full px-4 lg:px-6 xl:px-8 mt-2 flex flex-col gap-4">
          ...
          <main className="w-full px-4 lg:px-6 xl:px-8 mt-3 sm:mt-4 lg:mt-5 flex flex-col gap-4 flex-1 lg:min-h-0 lg:overflow-hidden pb-6 lg:pb-4">
            
            <div className="flex flex-col gap-4 mt-3 sm:mt-5 lg:mt-8 flex-1 min-h-0 overflow-y-auto no-scrollbar lg:pr-2">
              <div className="flex-shrink-0">
                <Filters filters={filters} setFilters={setFilters} />
              </div>

              <div className={`flex-col gap-2.5 flex-1 ${activeTab === 'list' ? 'flex' : 'hidden lg:flex'}`}>
                  {/* Selector Explorar / Favoritas */}
                  <div className="bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-2xl flex items-center mb-1">
                    <button 
                      onClick={() => setViewMode('explore')}
                      className={`flex-1 py-2 text-xs font-bold rounded-xl transition flex items-center justify-center gap-1.5 ${viewMode === 'explore' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}
                    >
                    ...
                    <button 
                      ...
                      className={`flex-1 py-2 text-xs font-bold rounded-xl transition flex items-center justify-center gap-1.5 ${viewMode === 'favorites' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}
                    >
                      <svg className={`w-3.5 h-3.5 ${viewMode === 'favorites' ? 'text-rose-500 fill-rose-500' : ''}`} fill={viewMode === 'favorites' ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                      Mis Favoritas
                    </button>
                  </div>
                  ...
                  {loading ? (
                    <div className="flex flex-col gap-4">
                      {[1,2,3].map(i => (
                        <div key={i} className="bg-white rounded-3xl h-32 animate-pulse shadow-sm"></div>
                      ))}
                    </div>
                  ) : (
                    <StationList 
              ...
              {/* Mapa para Móviles */}
              <div className={`native-card p-2 flex-col h-[500px] relative ${activeTab === 'map' ? 'flex lg:hidden' : 'hidden'}`}>
                <MapView 
                  userLocation={userLocation} 
                  stations={processedStations} 
                  minPrice={stats.min}
      ...
      <SubscriptionModal 
        ...
        onActivatePro={() => {
          saveSettings({ ...settings, isPro: true });
          setSubOpen(false);
        }}
      />
